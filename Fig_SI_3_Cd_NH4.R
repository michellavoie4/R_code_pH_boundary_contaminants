#-----------------------------------------------------------------------------#
# Fig. SI.3 Main program                                                      #
# Modeling Cd speciation in the boundary layer, NH4+ is the N source          #
#                                                                             #
# Lavoie M. 2018                                                              #
#-----------------------------------------------------------------------------#
...
# around spherical chrysophyte cells.     #
# Inclusion of Cd 2+, Cd(OH)+, and CdCO3(aq) 
# NH4+ is the N source and R = 5 and 30 um ; pH = 7, 5 and 8
...
boundary_Cd_NH4<- function(I, R, L, pH) {
  ...
  k_w <- 3E+08        # Water loss rate constants (in s-1) for Cd2+ (for formation of unidentate complexes) (Stumm and Morgan, 1996; Wilkinson et al 2004)
  ...
  K0_CdOH <- 10^3.91   # Thermodynamic equlibrium constant for CdOH+ formation (Martell et al. 2004)
  ...
  kf_CdCO3
  source("func_K_corr2_I.R")  # Loading the function for ionic strength correction of equilibrium constant (K0 to K1)
  K0_CdCO3 <- 10^4.36   # Thermodynamic equlibrium constant for CdCO3 formation (Martell et al 2004)
  ...
  # Concentration of CO2,HCO3,CO3,H,OH, Cd, CdOH, CdCO3 in bulk solution (mol m^-3) at I = I, T = 25 ?C and a given pH
...
#------------------------------------------------#
carb_pH7 <- boundary_Cd_NH4(I = 0.001, R = 5E-06 , L = 150E-06, pH = 7)
carb_pH5 <- boundary_Cd_NH4(I = 0.001, R = 5E-06 , L = 150E-06, pH = 5)
carb_pH8 <- boundary_Cd_NH4(I = 0.001, R = 5E-06 , L = 150E-06, pH = 8)

# Plot of relative change (C/Co) of each Cd species at different pHs.
tiff( "FigSI3_A.tiff", res = 100)
oldpar <- par(mfrow=c(2,2), mar=c(0,0,0,3), oma = c(4.1,4,4,0.4), las=1) 

# Panel A : Cd2+ concentrations (r = 5 um)
plot(carb_pH7$xlabel*1E+06-5, carb_pH7$Cdenrich, type = "l", lty = 1, xaxt = "n", xlim = c(0, 64), ylim = c(0.95, 1.05))
...
plot(carb_pH7$xlabel*1E+06-5, carb_pH7$CdOHenrich, type = "l", lty = 1, xlim = c(0, 64), ylim = c(0.7, 1.2))
...
plot(carb_pH7$xlabel*1E+06-5, carb_pH7$CdCO3enrich, type = "l", lty = 1, xlim = c(0, 64), ylim = c(0.7, 1.2))
...
#------------------------------------------------#
carb_pH7 <- boundary_Cd_NH4(I = 0.001, R = 30E-06 , L = 900E-06, pH = 7)
carb_pH5 <- boundary_Cd_NH4(I = 0.001, R = 30E-06 , L = 900E-06, pH = 5)
carb_pH8 <- boundary_Cd_NH4(I = 0.001, R = 30E-06 , L = 900E-06, pH = 8)

# Plot of relative change (C/Co) of each chemical species at different pHs.
tiff( "FigSI3_D.tiff", res = 100)
oldpar <- par(mfrow=c(2,2), mar=c(0,0,0,3), oma = c(4.1,4,4,0.4), las=1) 

# Panel D : Cd2+ concentrations (r = 30 um)
plot(carb_pH7$xlabel*1E+06-30, carb_pH7$Cdenrich, type = "l", lty = 1, xaxt = "n", cex.axis = 0.8, xlim = c(0, 110), ylim = c(0.95, 1.05))
...
plot(carb_pH7$xlabel*1E+06-30, carb_pH7$CdOHenrich, type = "l", lty = 1, cex.axis = 0.8, xlim = c(0, 110), ylim = c(0.3, 1.2))
...
plot(carb_pH7$xlabel*1E+06-30, carb_pH7$CdCO3enrich, type = "l", lty = 1, cex.axis = 0.8, xlim = c(0, 110), ylim = c(0.3, 1.1))
