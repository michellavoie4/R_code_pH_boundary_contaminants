#-----------------------------------------------------------------------------#
# Fig.3A    Main program                                                      #
# Modeling Clindamycin (neutral species) gradients in the boundary layer      #
#                                                                             #
# Lavoie M. 2018                                                              #
#-----------------------------------------------------------------------------#
...
#-------------------------------------------------------------------------#
# Plot (Fig3A) of the data overlayed with thermodynamic prediction        #
#-------------------------------------------------------------------------#

bmat <- unlist(boundary$y)
bmat1 <- as.matrix(bmat)
bmat1[,6]      # ClindH concentration
plot(X.grid$x.mid, bmat1[,6], type = "p", lty = 1, xlab = "radial distance", ylab = "Concentration of ClindH")   # Plot of ClindH as a function of distance

bmat1[,7]      # ClindH+ concentration
bmat1[,4]      # H+ concentration
# Calculation of neutral ClindH at thermodynamic equilibrium (using K_Clind1, ClindHplus and H+ concentration)
...
# Relative concentration units (plots at R = 30 um)
tiff("ClindH_30um_NO3.tiff", res = 100)
oldpar <- par(mfrow=c(1,1), mar = c(5,5,4,2))
ClindH_res <- bmat1[,6]
ClindHenrich <- ClindH_res / ClindH
plot(X.grid$x.mid*1E+06 - 30, ClindHenrich, type = "p", lty = 1, xlim = c(0,100), ylim = c(0,5), xlab = expression(paste("distance from cell surface (", mu, "m)")), ylab = expression(paste("Relative change in neutral Clind")))   # Plot of ClindH as a function of distance
